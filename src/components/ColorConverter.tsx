...
const ColorConverter = () => {
  ...
  const handleNameChange = useCallback((id: string, name: string) => {
    setColors(prev => prev.map(color => color.id === id ? {
      ...color,
      name: name || undefined
    } : color));
  }, []);
  ...
  return <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'dark bg-black' : 'bg-white'}`}>
      <div className="container mx-auto px-8 py-4 h-screen flex flex-col">
        <Card className="flex-1 flex flex-col bg-white dark:bg-black border-gray-200 dark:border-gray-800 shadow-lg">
          <CardHeader className="flex-shrink-0">
            <div className={`flex ${isMobile ? 'flex-col gap-4' : 'items-center justify-between'}`}>
              ...
              <div className={`flex items-center gap-4 ${isMobile ? 'justify-between' : ''}`}>
                ...
                <Button variant="outline" size="icon" onClick={() => setViewMode(viewMode === 'list' ? 'swatch' : 'list')} className="bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 hover:scale-110 transition-transform duration-200" title={`Switch to ${viewMode === 'list' ? 'swatch' : 'list'} view`}>
                  {viewMode === 'list' ? <Grid className="h-4 w-4" /> : <List className="h-4 w-4" />}
                </Button>
                <Button variant="outline" size="icon" onClick={toggleTheme} className="bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 hover:scale-110 transition-transform duration-200">
          ...
          <CardContent className="flex-1 flex flex-col overflow-hidden">
            <div className={`flex-1 overflow-auto ${viewMode === 'list' ? 'grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 auto-rows-min' : 'grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 auto-rows-min'}`}>
              {colors.map(color => viewMode === 'list' ? <ColorRow key={color.id} id={color.id} value={color.value} name={color.name} isValid={color.isValid} hasPreview={color.hasPreview} errorMessage={color.errorMessage} format={format} onValueChange={handleColorChange} onNameChange={handleNameChange} onCopy={handleCopyColor} onRemove={colors.length > 1 ? removeColor : undefined} /> : <ColorSwatch key={color.id} id={color.id} value={color.value} name={color.name} isValid={color.isValid} hasPreview={color.hasPreview} errorMessage={color.errorMessage} format={format} onValueChange={handleColorChange} onNameChange={handleNameChange} onCopy={handleCopyColor} onRemove={colors.length > 1 ? removeColor : undefined} />)}
              
              {colors.length < MAX_COLORS && <AddCard onClick={addColor} colorCount={colors.length} maxColors={MAX_COLORS} />}
            </div>
          </CardContent>
        </Card>
      </div>
